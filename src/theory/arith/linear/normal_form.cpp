...
bool VarList::isMember(Node n) { return Variable::isMember(n); }
                                        ...
                                        std::vector<Monomial>& monos)
{
  ...
  for (writePos = 0, readPos = 0, N = monos.size(); readPos < N;)
  {
    ...
    {
      if (!(varList == monos[rangeEnd].getVarList()))
      {
        break;
      }
    }
    // monos[i] for i in [readPos, rangeEnd) has the same var list
    if (readPos + 1 == rangeEnd)
    {  // no addition needed
...
{
  for (vector<Monomial>::const_iterator i = list.begin(), end = list.end();
       i != end;
       ++i)
  {
    const Monomial& m = *i;
...
Polynomial Polynomial::operator+(const Polynomial& vl) const
{
  ...
  std::merge(
      begin(), end(), vl.begin(), vl.end(), std::back_inserter(sortedMonos));
                                      ...
                                      const std::vector<Polynomial>& ps)
{
  ...
  else
  {
    ...
    std::vector<Monomial> monos;
    std::map<Node, Rational>::const_iterator ci = coeffs.begin(),
                                             cend = coeffs.end();
    for (; ci != cend; ++ci)
    {
      if (!(*ci).second.isZero())
...
bool Polynomial::numeratorGCDIsOne() const { return gcd().isOne(); }
...
Polynomial Comparison::normalizedVariablePart() const
{
  ...
  switch (cmpKind)
  {
    ...
    case Kind::DISTINCT:
    {
      ...
      {
        Polynomial noConstant =
            right.containsConstant() ? right.getTail() : right;
...
DeltaRational Comparison::normalizedDeltaRational() const
{
  ...
  switch (cmpKind)
  {
    ...
    case Kind::DISTINCT:
    {
      ...
      {  // The constant is 0 sign cannot change
...
/** This must be (not (>= qpolynomial constant)) or (not (>= zpolynomial
 * constant)) */
  ...
  if (pleft.numMonomials() == 1)
  {
    ...
    else
    {
      ...
      if (allIntegralVariables())
      {
        ...
        Polynomial varRight =
            pright.containsConstant() ? pright.getTail() : pright;
        if (lcoeff.sgn() <= 0)
        {
          return false;
        }
  ...
  else
  {
    ...
    switch (k)
    {
      ...
      case Kind::LT:
      {
        ...
        result = isInteger ? mkIntInequality(negKind, neg)
                           : mkRatInequality(negKind, neg);
      }
      break;
      case Kind::GEQ:
      case Kind::GT:
