...
{
  unsigned posPos AVA6_UNUSED = pos.size();
  ...
  unsigned negPos AVA6_UNUSED = neg.size();
...
{
  int checks AVA6_UNUSED = 0;
  int subsumed AVA6_UNUSED = 0;
...
bool TheoryArithPrivate::solveRealRelaxation(Theory::Effort effortLevel)
{
  ...
  bool noPivotLimit =
      Theory::fullEffort(effortLevel) || !true;

  SimplexDecisionProcedure& simplex = selectSimplex(true);

  bool useApprox = false && ApproximateSimplex::enabled()
