...
TheoryArithPrivate::~TheoryArithPrivate() = default;
...
bool TheoryArithPrivate::solveRelaxationOrPanic(Theory::Effort effortLevel)
{
  ...
  {
    d_qflraStatus = d_dualSimplex.findModel(false);
  }
  ...
  if (Theory::fullEffort(effortLevel) && d_qflraStatus == Result::UNKNOWN)
  {
    ...
    if (canBranch != ARITHVAR_SENTINEL)
    {
      ...
      {
        d_pendingBranches.push_back(branch);
        return true;
      }
    }
    d_qflraStatus = d_soiSimplex.findModel(true);
  }
  return false;
}
...
void TheoryArithPrivate::preNotifyFact(TNode fact)
{
  ...
  {
    bool res AVA6_UNUSED = assertionCases(curr);
...
bool TheoryArithPrivate::postCheck(Theory::Effort effortLevel)
{
  ...
  Trace("arith::ems") << "ems: " << emmittedConflictOrSplit << "pre pending branches"
                      << endl;
  if (!d_pendingBranches.empty())
  {
    bool anyFresh = false;
    while (!d_pendingBranches.empty())
    {
      TrustNode lem = d_pendingBranches.front();
      d_pendingBranches.pop();
      Trace("arith::branches") << "integer branch:" << lem << endl;
      anyFresh = anyFresh || hasFreshArithLiteral(lem.getNode());
      Trace("arith::lemma") << "integer branch:" << lem << endl;
      outputTrustedLemma(lem, InferenceId::ARITH_BB_LEMMA);
    }
    if (anyFresh)
    {
      emmittedConflictOrSplit = true;
