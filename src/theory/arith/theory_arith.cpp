...
bool TheoryArith::preCheck(AVA6_UNUSED Effort level)
{
...
void TheoryArith::propagate(AVA6_UNUSED Effort e) { d_internal.propagate(); }
...
bool TheoryArith::sanityCheckIntegerModel()
{
  ...
  {
    for (AVA6_UNUSED const auto& p : d_arithModelCache)
    {
      AssertEqual(p.first.getType(), p.second.getType())
          << "Bad type: " << p.first << " -> " << p.second;
    }
