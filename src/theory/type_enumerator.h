...
#ifndef AVA6__THEORY__TYPE_ENUMERATOR_H
#define AVA6__THEORY__TYPE_ENUMERATOR_H
...
/** String-alphabet configuration propagated to nested enumerators. */
class TypeEnumeratorProperties
{
 public:
  TypeEnumeratorProperties(uint32_t strAlphaCard)
      : d_stringAlphaCard(strAlphaCard)
  {
  }
...
#if defined(AVA6_ASSERTIONS) && !(defined(__clang__))
    if (d_te->isFinished())
    {
      try
      {
...
#endif /* AVA6_ASSERTIONS && !(APPLE || clang) */
...
#if defined(AVA6_ASSERTIONS) && !(defined(__APPLE__) && defined(__clang__))
    try
    {
      Node n = **d_te;
...
#else  /* AVA6_ASSERTIONS && !(APPLE || clang) */
    return **d_te;
#endif /* AVA6_ASSERTIONS && !(APPLE || clang) */
  }
...
#endif /* AVA6__THEORY__TYPE_ENUMERATOR_H */
