...
bool RelevantDomain::reset(AVA6_UNUSED Theory::Effort e)
{
  d_is_computed = false;
  return true;
}

void RelevantDomain::registerQuantifier(AVA6_UNUSED Node q) {}
                                              ...
                                              Node n)
{
  ...
  if (varCount == 2)
  {
    ...
    rdl.d_merge = (AVA6_EQUAL(n[0].getType(), n[1].getType()));
  }
  else if (varCount == 1)
  {
    rVar = n[varCh];
  ...
  else if (n[0].getType().isRealOrInt())
  {
    ...
    if (ArithMSum::getMonomialSumLit(n, msum))
    {
      ...
      if (!var.isNull())
      {
        ...
        else if (!hasNonVar && AVA6_EQUAL(var.getType(), var2.getType()))
        {
