...
void QuantInfo::debugPrintMatch(AVA6_UNUSED const char* c) const
{
  for (size_t i = 0, nvars = getNumVars(); i < nvars; i++)
  {
    Trace(c) << "  " << d_vars[i] << " -> ";
...
void MatchGen::debugPrintType(AVA6_UNUSED const char* c, short typ)
{
  switch (typ)
  {
...
{
  return TermUtil::isBoolConnectiveTerm(n);
}
...
void QuantConflictFind::reset_round(AVA6_UNUSED Theory::Effort level)
{
...
void QuantConflictFind::debugPrint(AVA6_UNUSED const char* c) const
{
...
void QuantConflictFind::debugPrintQuant(AVA6_UNUSED const char* c, Node q) const
{
