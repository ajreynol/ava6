...
TrustNode InstRewriterCegqi::rewriteInstantiation(
    AVA6_UNUSED Node q,
    AVA6_UNUSED const std::vector<Node>& terms,
    Node inst,
    bool doVts)
{
...
void InstStrategyCegqi::check(AVA6_UNUSED Theory::Effort e, QEffort quant_e)
{
...
bool InstStrategyCegqi::doCbqi(Node q)
{
  ...
  if (it == d_do_cbqi.end())
  {
    CegHandledStatus ret =
        CegInstantiator::isCbqiQuant(q, false);
