  ...
  else
  {
    ...
    if (c[index].getType().isUninterpretedSort())
    {
      ...
      {  /// option fmfFmcCoverSimplify
        ...
        {
          bool complete = true;
          for (std::map<Node, EntryTrie>::iterator it = d_child.begin();
               it != d_child.end();
               ++it)
          {
...
int EntryTrie::getGeneralizationIndex(FirstOrderModelFmc* m,
...
void EntryTrie::addEntry(
    FirstOrderModelFmc* m, Node c, Node v, int data, int index)
{
  if (index == (int)c.getNumChildren())
  {
    if (d_data == -1)
...
void EntryTrie::getEntries(FirstOrderModelFmc* m,
                           Node c,
...
{
  Trace("fmc-simplify") << "Simplify definition, #cond = " << d_cond.size()
                        << std::endl;
  basic_simplify(m);
  Trace("fmc-simplify") << "post-basic simplify, #cond = " << d_cond.size()
                        << std::endl;

  // check if the last entry is not all star, if so, we can make the last entry
  // all stars
  if (!d_cond.empty())
  {
  ...
  Trace("fmc-simplify") << "finish simplify, #cond = " << d_cond.size()
                        << std::endl;
}
...
bool FullModelChecker::preProcessBuildModel(TheoryModel* m)
{
  ...
  // do not have to introduce terms for sorts of domains of quantified formulas
  // if we are allowed to assume empty sorts
...
void FullModelChecker::initializeFunctions(TheoryModel* m)
{
       ...
       ++it)
  {
    ...
    {
      Trace("fmc") << "Cardinality( " << it->first << " )" << " = "
  ...
  for (std::pair<const Node, Def*>& fmm : d_fm->d_models)
  {
    ...
    {
      Trace("fmc-model-debug")
          << "No model values for " << op << " ... " << std::endl;
    }
    Trace("fmc-model-debug") << std::endl;
    ...
    if (needsDefault)
    {
      ...
      else
      {
        ...
        Trace("fmc-model-debug")
            << "Add default to default representative " << nmb << " ";
        Trace("fmc-model-debug")
    ...
    for (size_t i = 0; i < add_conds.size(); i++)
    {
      ...
      Node en = hasNonStar
                    ? n
                    : nodeManager()->mkNode(Kind::APPLY_UF, entry_children);
      if (std::find(conds.begin(), conds.end(), n) == conds.end())
      {
        Trace("fmc-model-debug") << "- add " << n << " -> " << nv
                                 << " (entry is " << en << ")" << std::endl;
        conds.push_back(n);
        values.push_back(nv);
      ...
      {
        Trace("fmc-model-debug")
            << "Already have entry for : " << n << " -> " << nv << " (entry is "
            << en << ")" << std::endl;
      }
    }
    ...
    {
      fm->d_models[op]->addEntry(
          fm, entry_conds[indices[i]], values[indices[i]]);
    }
  ...
  for (std::map<Node, Def*>::iterator it = fm->d_models.begin();
       it != fm->d_models.end();
       ++it)
  {
...
void FullModelChecker::debugPrint(AVA6_UNUSED const char* tr,
                                  Node n,
                                  bool dispStar)
{
  if (n.isNull())
...
int FullModelChecker::doExhaustiveInstantiation(FirstOrderModel* fm,
                                                Node f,
                                                int effort)
{
  // ensure that functions are assigned
  initializeFunctions(fm->getTheoryModel());
  Trace("fmc") << "Full model check " << f << ", effort = " << effort << "..."
               << std::endl;
  ...
  if (riter.setQuantifier(f))
  {
    ...
    while (!riter.isFinished())
    {
      ...
      Trace("fmc-exh-debug") << ", index = " << ev_index << " / "
                             << d_quant_models[f].d_value.size();
      Node ev =
          ev_index == -1 ? Node::null() : d_quant_models[f].d_value[ev_index];
      if (ev != d_true)
      {
    ...
    Trace("fmc-exh") << "----Finished Exhaustive instantiate, lemmas = "
                     << addedLemmas << ", incomplete=" << riter.isIncomplete()
                     << std::endl;
    return addedLemmas > 0 || !riter.isIncomplete();
  }
  else
  {
    Trace("fmc-exh") << "----Finished Exhaustive instantiate, failed."
                     << std::endl;
...
void FullModelChecker::doCheck(FirstOrderModelFmc* fm, Node f, Def& d, Node n)
{
  ...
  {
    Trace("fmc-debug") << "It is a bounding literal, polarity = "
                       << n.getAttribute(BoundIntLitAttribute()) << std::endl;
    d.addEntry(fm,
               mkCondDefault(fm, f),
               n.getAttribute(BoundIntLitAttribute()) == 1 ? d_false : d_true);
  }
  ...
  {
    // Trace("fmc-warn") << "WARNING : ARRAYS : Can't process base array " << r
    // << std::endl; Trace("fmc-warn") << "          Default value was : " <<
    // odefaultValue << std::endl; Trace("fmc-debug") << "Can't process base
    // array " << r << std::endl; can't process this array
    d.reset();
    d.addEntry(fm, mkCondDefault(fm, f), Node::null());
  }
...
void FullModelChecker::doVariableEquality(FirstOrderModelFmc* fm,
                                          Node f,
                                          Def& d,
                                          Node eq)
{
...
void FullModelChecker::doVariableRelation(
...
void FullModelChecker::doUninterpretedCompose(
    FirstOrderModelFmc* fm, Node f, Def& d, Node op, std::vector<Def>& dc)
{
...
void FullModelChecker::doUninterpretedCompose(FirstOrderModelFmc* fm,
                                              Node f,
                                              Def& d,
                                              Def& df,
                                              std::vector<Def>& dc,
  ...
  else
  {
    for (unsigned i = 0; i < dc[index].d_cond.size(); i++)
    {
      if (isCompat(fm, cond, dc[index].d_cond[i]) != 0)
      {
        ...
        {
          Trace("fmc-uf-process")
              << "index " << i << " succeeded meet." << std::endl;
...
void FullModelChecker::doUninterpretedCompose2(FirstOrderModelFmc* fm,
                                               Node f,
                                               std::map<int, Node>& entries,
                                               int index,
                                               std::vector<Node>& cond,
                                               ...
                                               EntryTrie& curr)
{
  ...
  {
    Node c = mkCond(cond);
    Trace("fmc-uf-entry") << "Entry : " << c << " -> index[" << curr.d_data
                          << "]" << std::endl;
...
void FullModelChecker::doInterpretedCompose(FirstOrderModelFmc* fm,
                                            Node f,
                                            Def& d,
                                            Node n,
                                            std::vector<Def>& dc,
...
int FullModelChecker::isCompat(FirstOrderModelFmc* fm,
                               std::vector<Node>& cond,
                               Node c)
{
...
bool FullModelChecker::doMeet(FirstOrderModelFmc* fm,
                              std::vector<Node>& cond,
                              Node c)
{
...
void FullModelChecker::mkCondDefaultVec(FirstOrderModelFmc* fm,
                                        Node f,
                                        std::vector<Node>& cond)
{
...
Node FullModelChecker::getFunctionValue(FirstOrderModelFmc* fm,
                                        Node op,
...
void FullModelChecker::registerQuantifiedFormula(Node q)
{
  ...
  Node op = NodeManager::mkDummySkolem("qfmc", typ);
  d_quant_cond[q] = op;
}
