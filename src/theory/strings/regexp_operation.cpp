...
// 0-unknown, 1-yes, 2-no
int RegExpOpr::derivativeS(Node r, ava6::internal::String c, Node& retNode)
{
  Assert(c.size() < 2);
  ...
  else
  {
    switch (r.getKind())
    {
      ...
      case Kind::REGEXP_RANGE:
      {
        ava6::internal::String a = r[0].getConst<String>();
        ava6::internal::String b = r[1].getConst<String>();
...
Node RegExpOpr::derivativeSingle(Node r, ava6::internal::String c)
{
  Assert(c.size() < 2);
  Trace("regexp-derive") << "RegExp-derive starts with /" << mkString(r)
                         << "/, c=" << c << std::endl;
