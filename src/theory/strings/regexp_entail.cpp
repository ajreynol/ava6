  ...
  for (unsigned t = 0; t < 2; t++)
  {
    if (tmin <= t && t <= tmax)
    {
      ...
      while (!children.empty() && !mchildren.empty() && do_next)
      {
        ...
        {
          // check for constants
          ava6::internal::String s = xc.getConst<String>();
          if (Word::isEmpty(xc))
          {
  ...
  switch (k)
  {
    ...
    case Kind::REGEXP_STAR:
    {
      if (s.size() != index_start)
      {
        ...
        {
          ava6::internal::String t = s.substr(index_start, i);
    ...
    case Kind::REGEXP_LOOP:
    {
      ...
      else
      {
        ...
        if (l == 0)
        {
          ...
          {
            ava6::internal::String t = s.substr(index_start, len);
            if (testConstStringInRegExpInternal(t, 0, r[0]))
            {
              ...
              {
                Node num2 = nm->mkConstInt(ava6::internal::Rational(u - 1));
        ...
        else
        {
          ...
          for (unsigned len = 1; len <= s.size() - index_start; len++)
          {
            ...
            if (testConstStringInRegExpInternal(t, 0, r[0]))
            {
              Node num2 = nm->mkConstInt(ava6::internal::Rational(l - 1));
