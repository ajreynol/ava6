...
using namespace std;
using namespace ava6::internal::expr;
...
NodeManager::~NodeManager()
{
  ...
  // clear the datatypes and declared sorts
  d_dtypes.clear();
  d_nfreshSorts.clear();
  d_nfreshVars.clear();
       ...
       ++i)
  {
    ...
    if (nv->d_rc == 0)
    {
      ...
      if (mk == kind::metakind::CONSTANT)
      {
        ...
        // e.g. ava6::internal::Rational, since it has a gmp internal
...
{
  return k == Kind::ABSTRACT_TYPE || k == Kind::ARRAY_TYPE || k == Kind::BITVECTOR_TYPE
         || k == Kind::TUPLE_TYPE || k == Kind::FUNCTION_TYPE
    ...
    const std::set<TypeNode>& unresolvedTypes)
{
  ...
  for (const TypeNode& ut : dtts)
  {
    ...
    for (size_t i = 0, ncons = dt.getNumConstructors(); i < ncons; i++)
    {
      ...
      TypeNode testerType AVA6_UNUSED = c.getTester().getType();
      ...
      TypeNode ctorType AVA6_UNUSED = c.getConstructor().getType();
      ...
      for (size_t j = 0, nargs = c.getNumArgs(); j < nargs; j++)
      {
        ...
        // ava6::internal::DType class, but this actually needs to be checked.
