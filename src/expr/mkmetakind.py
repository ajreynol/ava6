...
class CodeGenerator:
    ...
    def register_constant(self, constant, filename):
        ...
        if not re.match(r'::.*', hasher):
            print(
                f"{filename}: warning: constant {name} hasher `{hasher}` isn't fully-qualified (e.g., ::ava6::internal::RationalHashFunction)"
            )
...
{class_name} const& NodeValue::getConst< {class_name} >() const {{
  //AssertArgument(getKind() == ::ava6::internal::Kind::{name}, *this,
...
{{
  return mkConstInternal<Node, {class_name}>(::ava6::internal::Kind::{name}, val);
}}
"""
            self.metakind_mkConst += f"""
template<>
