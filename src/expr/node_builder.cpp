...
{
  if (AVA6_PREDICT_FALSE(nvIsAllocated()))
  {
    dealloc();
  }
  else if (AVA6_PREDICT_FALSE(!isUsed()))
  {
    decrRefCounts();
  }
}
...
NodeBuilder& NodeBuilder::operator<<(const Kind& k)
{
  ...
  if (AVA6_PREDICT_FALSE(d_nv->d_id == 0 && getKind() != Kind::UNDEFINED_KIND))
  {
...
expr::NodeValue* NodeBuilder::constructNV()
{
  ...
  if (AVA6_PREDICT_TRUE(!nvIsAllocated()))
  {
...
void NodeBuilder::internalCopy(const NodeBuilder& nb)
{
  ...
  bool realloced AVA6_UNUSED = false;
...
bool NodeBuilder::isUsed() const { return AVA6_PREDICT_FALSE(d_nv == nullptr); }
...
{
  return AVA6_PREDICT_FALSE(d_nv != &d_inlineNv)
         && AVA6_PREDICT_TRUE(d_nv != nullptr);
}

bool NodeBuilder::nvNeedsToBeAllocated() const
{
  return AVA6_PREDICT_FALSE(d_nv->d_nchildren == d_nvMaxChildren);
}
...
void NodeBuilder::realloc()
{
  ...
  realloc(AVA6_PREDICT_FALSE(newSize > hardLimit) ? hardLimit : newSize);
}

void NodeBuilder::allocateNvIfNecessaryForAppend()
{
  if (AVA6_PREDICT_FALSE(nvNeedsToBeAllocated()))
  {
    realloc();
  }
}
...
{
  if (AVA6_PREDICT_FALSE(nvIsAllocated())
      && AVA6_PREDICT_TRUE(d_nvMaxChildren > d_nv->d_nchildren))
  {
    // Ensure d_nv is not modified on allocation failure
    expr::NodeValue* newBlock = (expr::NodeValue*)std::realloc(
        d_nv,
