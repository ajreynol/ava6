...
namespace std {
size_t hash<ava6::internal::SygusGrammar>::operator()(
    const ava6::internal::SygusGrammar& grammar) const
{
  uint64_t ret = ava6::internal::fnv1a::offsetBasis;
  for (const auto& v : grammar.d_sygusVars)
  {
    ret = ava6::internal::fnv1a::fnv1a_64(ret,
                                          std::hash<ava6::internal::Node>{}(v));
  }
  for (const auto& nts : grammar.d_ntSyms)
  {
    ret = ava6::internal::fnv1a::fnv1a_64(
        ret, std::hash<ava6::internal::Node>{}(nts));
  }
  for (const auto& r : grammar.d_rules)
  {
    uint64_t rhash = ava6::internal::fnv1a::offsetBasis;
    for (const auto& n : r.second)
    {
      rhash = ava6::internal::fnv1a::fnv1a_64(
          rhash, std::hash<ava6::internal::Node>{}(n));
    }
    rhash = ava6::internal::fnv1a::fnv1a_64(
        rhash, std::hash<ava6::internal::Node>{}(r.first));
    ret = ava6::internal::fnv1a::fnv1a_64(ret, rhash);
  }
  return ava6::internal::fnv1a::fnv1a_64(
      ret, std::hash<ava6::internal::TypeNode>{}(grammar.d_datatype));
}
}  // namespace std
