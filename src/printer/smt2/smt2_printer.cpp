                               ...
                               int toDepth) const
{
  ...
  if (n.getMetaKind() == kind::metakind::CONSTANT)
  {
    switch (n.getKind())
    {
      ...
      case Kind::UNINTERPRETED_SORT_VALUE:
      {
        ...
        out << "(as " << ava6::internal::quoteSymbol(v.getSymbol()) << " "
            << n.getType() << ")";
        break;
      }
      ...
      || k == Kind::NULLABLE_TYPE)
  {
    ...
    {
      out << ava6::internal::quoteSymbol(dt.getName());
    }
    return true;
  }
  ...
  else if (n.isVar())
  {
      ...
      if (sm->isSkolemFunction(n, id, cacheVal))
      {
        if (id == SkolemId::INTERNAL)
        {
          if (sm->isAbstractValue(n))
          {
            ...
            out << "(as " << ava6::internal::quoteSymbol(s) << " "
                << n.getType() << ")";
            printed = true;
          }
        }
    ...
    if (!printed)
    {
      ...
      if (n.hasName())
      {
        ...
        {
          out << ava6::internal::quoteSymbol(s);
        }
      }
      else
      {
...
std::string Smt2Printer::smtKindStringOf(const Node& n)
{
  ...
  {
    // this method parallels ava6::Term::getKind
    switch (k)
    {
...
void Smt2Printer::toStream(std::ostream& out, const UnsatCore& core) const
{
  ...
  if (core.useNames())
  {
    ...
    {
      out << ava6::internal::quoteSymbol(cn) << std::endl;
    }
  }
  else
  {
                                    ...
                                    const std::vector<Node>& elements) const
{
  ...
  if (modelUninterpPrint == options::ModelUninterpPrintMode::Datatype)
  {
    ...
    for (size_t i = 0, nelements = elements.size(); i < nelements; i++)
    {
      ...
      out << "(" << ava6::internal::quoteSymbol(av.getSymbol()) << ")";
    }
    out << "))" << std::endl;
    return;
  }
...
{
  out << "(error " << ava6::internal::quoteString(message) << ')' << endl;
}
    ...
    TypeNode type) const
{
  ...
  {
    out << "(declare-const " << ava6::internal::quoteSymbol(id);
    if (!argTypes.empty())
    {
      out << " (->";
  ...
  out << "(declare-fun " << ava6::internal::quoteSymbol(id) << " ";
                                            ...
                                            Node formula) const
{
  if (d_variant == Variant::eo_variant)
  {
    out << "(define " << ava6::internal::quoteSymbol(id) << " ";
    toStreamSortedVarList(out, formals);
    out << " " << formula << ')';
    return;
  }
  out << "(define-fun " << ava6::internal::quoteSymbol(id) << " ";
  toStreamSortedVarList(out, formals);
  out << " " << range << ' ' << formula << ')';
}
...
void Smt2Printer::toStreamCmdDeclareType(std::ostream& out,
                                         ...
                                         size_t arity) const
{
  out << "(declare-sort " << ava6::internal::quoteSymbol(id) << " " << arity
      << ")";
}
...
void Smt2Printer::toStreamCmdDefineType(std::ostream& out,
                                        ...
                                        TypeNode t) const
{
  out << "(define-sort " << ava6::internal::quoteSymbol(id) << " (";
  if (params.size() > 0)
  {
    copy(
...
void Smt2Printer::toStream(std::ostream& out, const DType& dt) const
{
  for (size_t i = 0, ncons = dt.getNumConstructors(); i < ncons; i++)
  {
    ...
    out << "(" << ava6::internal::quoteSymbol(cons.getName());
    ...
    std::ostream& out, const std::vector<TypeNode>& datatypes) const
{
  ...
  for (const TypeNode& t : datatypes)
  {
    ...
    out << "(" << ava6::internal::quoteSymbol(d.getName());
...
void Smt2Printer::toStreamCmdEmpty(AVA6_UNUSED std::ostream& out,
...
{
  out << "(echo " << ava6::internal::quoteString(output) << ')';
}
...
{
  out << "(synth-fun " << ava6::internal::quoteSymbol(id) << ' ';
  // print variable list
...
void Smt2Printer::toStreamCmdDeclareVar(std::ostream& out,
                                        ...
                                        TypeNode type) const
{
  out << "(declare-var " << ava6::internal::quoteSymbol(id) << ' ' << type
      << ')';
}
...
void Smt2Printer::toStreamCmdGetInterpol(std::ostream& out,
                                         ...
                                         TypeNode sygusType) const
{
  out << "(get-interpolant " << ava6::internal::quoteSymbol(name) << ' '
      << conj;
  if (!sygusType.isNull())
  {
    out << ' ' << sygusGrammarString(sygusType);
