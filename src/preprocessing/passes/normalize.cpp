      ...
      else
      {
        if (n.hasOperator())
        {
          ...
          if (opNode.isVar())
          {
            ...
            if (role.find(symbol) == role.end()
                && n.getKind() != ava6::internal::Kind::INST_ATTRIBUTE)
            {
              role[symbol] = cnt;
            }
            cnt++;  // Increment cnt whether variable is new or not
        ...
        ava6::internal::Kind k = static_cast<ava6::internal::Kind>(n.getKind());
        std::string opKind = ava6::internal::kind::toString(k);
        nodeEncoding += opKind + ",";
  ...
  while (!stack.empty())
  {
    ...
    if (inserted)
    {
      ...
      if (current.isConst() || current.isVar())
      {
        ...
        if (current.isVar())
        {
          if (current.getKind() == ava6::internal::Kind::BOUND_VARIABLE)
          {
      ...
      else
      {
        ...
        if (!hasQID
            && current.getKind() == ava6::internal::Kind::INST_ATTRIBUTE)
        {
        ...
        if (current.getKind() == ava6::internal::Kind::FORALL
            || current.getKind() == ava6::internal::Kind::EXISTS)
        {
          ...
          Node normalizedBoundVarList = nodeManager->mkNode(
              ava6::internal::Kind::BOUND_VAR_LIST, normalizedBoundVars);
        ...
        if (current.getKind() == ava6::internal::Kind::APPLY_UF)
        {
...
{
  if (n.getKind() == ava6::internal::Kind::EQUAL)
  {
  ...
  while (!stack.empty())
  {
    ...
    if (inserted)
    {
      ...
      if (current.isConst() || current.isVar())
      {
        ...
        if (current.isVar()
            && (!parent.isNull()
                && parent.getKind() == ava6::internal::Kind::INST_ATTRIBUTE))
        {
