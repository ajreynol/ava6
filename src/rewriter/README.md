...
This section pertains only to parsing RARE rules in ava6.
...
## Updating RARE rules in the ava6 repository

RARE rules are contained in the ava6 repository via files named
`src/theory/*/rewrites*`. These can be modified by ava6 developers when new
rewrites are deemed necessary. It is recommended that the entire ava6 team is
...
by our build system. We provide a script for this purpose, also in the ava6
repository at `./contrib/install-rare-rewrites`.

In particular, this script updates the `ProofRewriteRule` identifiers in
ava6's API, as well as the CPC proof signature.
