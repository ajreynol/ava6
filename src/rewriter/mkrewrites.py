...
def validate_rule(rule):
    ...
    if os.getenv('AVA6_RARE_CHECK_CONST', None) is not None and lhsHasConst:
...
def gen_rewrite_db(args):
    ...
    src_include_dir = os.path.join(args.src_dir, 'include', 'ava6')
    src_api_dir = os.path.join(args.src_dir, 'src', 'api', 'cpp')
    bin_include_dir = os.path.join(args.bin_dir, 'include', 'ava6')
    ...
    rewrites_files = args.rewrites_files
    for rewrites_file in rewrites_files:
    ...
    ava6_proof_rule_h = read_tpl_enclosed(src_include_dir, 'ava6_proof_rule.h')
    with open(os.path.join(bin_include_dir, 'ava6_proof_rule.h'), 'w') as f:
        f.write(ava6_proof_rule_h.format(
            rules='\n'.join([f'{doc(id)}\n  EVALUE({id}),' for id in ids])))

    ava6_proof_rule_cpp = read_tpl(src_api_dir, 'ava6_proof_rule_template.cpp')
    os.makedirs(bin_api_dir, exist_ok=True)
    with open(os.path.join(bin_api_dir, 'ava6_proof_rule.cpp'), 'w') as f:
        f.write(format_cpp(ava6_proof_rule_cpp.format(
            printer='\n'.join(printer_code))))
