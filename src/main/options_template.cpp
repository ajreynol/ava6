...
// clean up
#ifdef AVA6_IS_NOT_REALLY_BSD
#undef _BSD_SOURCE
#endif /* AVA6_IS_NOT_REALLY_BSD */
...
R"FOOBAR(Most commonly-used ava6 options:
${help_common}$
)FOOBAR";

static const std::string additionalOptionsDescription =
R"FOOBAR(Additional ava6 options:
${help_others}$
)FOOBAR";

static const std::string regularOptionsDescription =
R"FOOBAR(Regular ava6 options:
${help_regular}$
)FOOBAR";
...
void printUsage(const std::string& binary, std::ostream& os, bool printRegular)
{
  os << "usage: " << binary << " [options] [input-file]" << std::endl
     << std::endl
     << "Without an input file, or with `-', ava6 reads from standard "
        "input."
     << std::endl
     << std::endl
     << "ava6 options:" << std::endl
...
void parseInternal(ava6::Solver& solver,
                   int argc,
                   char* argv[],
...
/**
 * Parse argc/argv and put the result into a ava6::internal::Options.
