...
{
  safe_print(STDERR_FILENO, "ava6 interrupted by timeout.\n");
  print_statistics();
  abort();
}
...
void timeout_handler(AVA6_UNUSED int sig, AVA6_UNUSED siginfo_t* info, void*)
{
  timeout_handler();
}

/** Handler for SIGTERM. */
void sigterm_handler(int sig, AVA6_UNUSED siginfo_t* info, void*)
{
  safe_print(STDERR_FILENO, "ava6 interrupted by SIGTERM.\n");
  print_statistics();
  signal(sig, SIG_DFL);
  raise(sig);
}
...
void sigint_handler(int sig, AVA6_UNUSED siginfo_t* info, void*)
{
  safe_print(STDERR_FILENO, "ava6 interrupted by user.\n");
  print_statistics();
  signal(sig, SIG_DFL);
  raise(sig);
}
...
void segv_handler(int sig, AVA6_UNUSED siginfo_t* info, void*)
{
  uintptr_t extent = reinterpret_cast<uintptr_t>(stackBase) - stackSize;
  uintptr_t addr = reinterpret_cast<uintptr_t>(info->si_addr);
#ifdef AVA6_DEBUG
  safe_print(STDERR_FILENO, "ava6 suffered a segfault in DEBUG mode.\n");
...
#else  /* AVA6_DEBUG */
  safe_print(STDERR_FILENO, "ava6 suffered a segfault.\n");
  safe_print(STDERR_FILENO, "Offending address is ");
  safe_print(STDERR_FILENO, info->si_addr);
  safe_print(STDERR_FILENO, "\n");
  if (addr >= extent && addr <= extent + 10 * 1024)
...
void ill_handler(int sig, AVA6_UNUSED siginfo_t* info, void*)
{
#ifdef AVA6_DEBUG
  safe_print(STDERR_FILENO,
             "ava6 executed an illegal instruction in DEBUG mode.\n");
  if (!segvSpin)
  {
    print_statistics();
    signal(sig, SIG_DFL);
...
#else  /* AVA6_DEBUG */
  safe_print(STDERR_FILENO, "ava6 executed an illegal instruction.\n");
  print_statistics();
  signal(sig, SIG_DFL);
  raise(sig);
#endif /* AVA6_DEBUG */
...
void ava6terminate()
{
  ...
  safe_print(STDERR_FILENO,
             "\n"
             "ava6 was terminated by the C++ runtime.\n"
...
void install()
{
  ...
  default_terminator = set_terminate(ava6terminate);
}
