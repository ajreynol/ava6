 ...
 * ****************************************************************************
 *
 * The main entry point into the ava6 library's SMT interface.
 */
...
using namespace std;
using namespace ava6::internal::smt;
using namespace ava6::internal::preprocessing;
using namespace ava6::internal::prop;
...
SolverEngine::~SolverEngine()
{
  ...
  {
    d_env->warning() << "ava6 threw an exception during cleanup." << std::endl
                     << e << std::endl;
  }
}
...
void SolverEngine::debugCheckFunctionBody(Node formula,
                                          ...
                                          Node func)
{
  TypeNode formulaType = formula.getType(true);
...
void SolverEngine::declareConst(AVA6_UNUSED const Node& c)
{
  d_state->notifyDeclaration();
}

void SolverEngine::declareSort(AVA6_UNUSED const TypeNode& tn)
{
  d_state->notifyDeclaration();
}
                                  ...
                                  bool global)
{
  ...
  // eliminate subtypes (mixed arithmetic) if proof producing, see ava6#12754
