 ...
 * ****************************************************************************
 *
 * SolverEngine: the main public entry point of libava6.
 */

#include "ava6_public.h"

#ifndef AVA6__SMT__SOLVER_ENGINE_H
#define AVA6__SMT__SOLVER_ENGINE_H
...
class AVA6_EXPORT SolverEngine
{
  friend class ava6::Solver;
 ...
 public:
   ...
   * query). Only permitted if ava6 was built with unsat-core support and
   * produce-unsat-cores is on.
   */
  UnsatCore getUnsatCore();

  /** Get the lemmas used to derive UNSAT. Only permitted if ava6 was built with
   ...
   * Only permitted if ava6 was built with proof support and the proof option
   * is on.
   */
   ...
   * versions of ava6 (as may the number of conflicts required, anyway),
   * and it might even be different between instances of the same version
   * of ava6 on different platforms.
   *
 ...
 private:
  ...
  void setSolver(ava6::Solver* solver) { d_solver = solver; }
   ...
   * by an UNSAT query). Only permitted if ava6 was built with unsat-core
   ...
   * by this class is currently available, which means that ava6 is producing
  ...
  /**
   * Check that a model is available, i.e. that ava6 is producing models and is
