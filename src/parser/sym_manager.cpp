...
using namespace ava6::internal::parser;
...
{
  using TermStringMap =
      CDHashMap<ava6::Term, std::string, std::hash<ava6::Term>>;
  using TermSet = CDHashSet<ava6::Term, std::hash<ava6::Term>>;
  using SortList = CDList<ava6::Sort>;
  using TermList = CDList<ava6::Term>;
 ...
 public:
  ...
  /** set expression name */
  NamingResult setExpressionName(ava6::Term t,
                                 const std::string& name,
                                 bool isAssertion = false);
  /** get expression name */
  bool getExpressionName(ava6::Term t,
                         std::string& name,
                         bool isAssertion = false) const;
  /** get expression names */
  void getExpressionNames(const std::vector<ava6::Term>& ts,
                          std::vector<std::string>& names,
                          bool areAssertions = false) const;
  /** get expression names */
  std::map<ava6::Term, std::string> getExpressionNames(
...
NamingResult SymManager::Implementation::setExpressionName(
    ava6::Term t, const std::string& name, bool isAssertion)
{
...
bool SymManager::Implementation::getExpressionName(ava6::Term t,
                                                   std::string& name,
...
void SymManager::Implementation::getExpressionNames(
    const std::vector<ava6::Term>& ts,
    std::vector<std::string>& names,
    bool areAssertions) const
{
  for (const ava6::Term& t : ts)
  {
    std::string name;
...
std::vector<ava6::Sort> SymManager::Implementation::getDeclaredSorts() const
{
  std::vector<ava6::Sort> declareSorts(d_declareSorts.begin(),
                                       d_declareSorts.end());
  return declareSorts;
}

std::vector<ava6::Term> SymManager::Implementation::getDeclaredTerms() const
{
  std::vector<ava6::Term> declareTerms(d_declareTerms.begin(),
                                       d_declareTerms.end());
  return declareTerms;
}
...
void SymManager::Implementation::addModelDeclarationTerm(ava6::Term t)
{
...
SymManager::SymManager(ava6::TermManager& tm)
    : d_tm(tm),
...
bool SymManager::bind(const std::string& name, ava6::Term obj, bool doOverload)
{
  return d_implementation->getSymbolTable().bind(name, obj, doOverload);
}

bool SymManager::bindType(const std::string& name, ava6::Sort t, bool isUser)
{
  if (isUser && !d_termSortOverload)
  {
    ...
    ava6::Sort dummyType = d_tm.mkUninterpretedSort("Type");
    ava6::Term ts = d_tm.mkConst(dummyType, name);
...
bool SymManager::bindMutualDatatypeTypes(
    const std::vector<ava6::Sort>& datatypes, bool bindTesters)
{
...
NamingResult SymManager::setExpressionName(ava6::Term t,
                                           const std::string& name,
                                           bool isAssertion)
{
...
bool SymManager::getExpressionName(ava6::Term t,
                                   std::string& name,
                                   bool isAssertion) const
{
...
void SymManager::getExpressionNames(const std::vector<ava6::Term>& ts,
                                    std::vector<std::string>& names,
                                    bool areAssertions) const
{
...
std::map<ava6::Term, std::string> SymManager::getExpressionNames(
    bool areAssertions) const
{
  return d_implementation->getExpressionNames(areAssertions);
}
std::vector<ava6::Sort> SymManager::getDeclaredSorts() const
{
  return d_implementation->getDeclaredSorts();
}
std::vector<ava6::Term> SymManager::getDeclaredTerms() const
{
