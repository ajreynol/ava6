...
void Parser::setLogic(AVA6_UNUSED const std::string& name) {}
                                         ...
                                         SymManager* sm)
{
...
)
  {
    ...
    std::string mode = solver->getOption("strict-parsing") == "true" ? "strict" : "default";
    if (mode == "strict")
    {
    ...
    parser.reset(new Smt2Parser(solver, sm, parsingMode));
  }
  else
  {
    throw Ava6ApiException("Only the SMT-LIB text language is supported");
  }
  return parser;
}
