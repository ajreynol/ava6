...
void EoPrintChannelPre::printTypeNode(AVA6_UNUSED TypeNode tn)
{
  // current do nothing
}
...
void EoPrintChannelPre::printAssume(TNode n,
                                    ...
                                    AVA6_UNUSED bool isPush)
{
  processInternal(n);
}

void EoPrintChannelPre::printStep(
    AVA6_UNUSED const std::string& rname,
    TNode n,
    AVA6_UNUSED size_t i,
    AVA6_UNUSED const std::vector<size_t>& premises,
    const std::vector<Node>& args,
    AVA6_UNUSED bool isPop)
{
  if (!n.isNull())
  {
    processInternal(n);
