...
class Tester:
    ...
    def run_internal(self, benchmark_info):
        """Runs ava6 on a given benchmark and checks the output."""
...
# the ava6 binary with a set of arguments and checks the expected output (both
...
class CpcTesterBase(Tester):
    ...
    def applies(self, benchmark_info):
        return (
            benchmark_info.expected_output.strip() == "unsat"
        )

    def gen_proof(self, benchmark_info, ava6_args):
        """Runs ava6 to generate a CPC proof for the given benchmark. Returns
...
class CpcTester(CpcTesterBase):
    ...
    def run_internal(self, benchmark_info):
        with tempfile.NamedTemporaryFile() as tmpf:
            ...
            proof, exit_code = self.gen_proof(benchmark_info, ava6_args)
            if exit_code != EXIT_OK:
                return exit_code
...
class ModelTester(Tester):
    ...
    def applies(self, benchmark_info):
        expected_output_lines = benchmark_info.expected_output.split()
        return (("sat" in expected_output_lines
...
CTEST_TIMEOUT_ENV = "AVA6_REGRESSION_TIMEOUT_AS_CTEST_TIMEOUT"
CTEST_TIMEOUT_MARKER = "AVA6_REGRESSION_CTEST_TIMEOUT"
CTEST_TIMEOUT_SLEEP = 2
# Some testers run ava6 plus an external checker or scrubbers.
...
def get_ava6_features(ava6_binary, timeout):
    """Returns a list of features supported by the ava6 binary `ava6_binary`."""

    output, error, exit_status = run_process([ava6_binary, "--show-config"],
                                             None, timeout)
    if is_timeout(exit_status, output, error):
        print_error("Timeout")
        return None, None
...
def run_benchmark(benchmark_info):
    """Runs ava6 on a benchmark with the given `benchmark_info`. It runs on the
    file `benchmark_basename` in the directory `benchmark_dir` using the binary
    `ava6_binary` with the command line options `command_line_args`. The output
    is scrubbed using `scrubber` and `error_scrubber` for stdout and stderr,
    respectively."""

    bin_args = benchmark_info.wrapper[:]
    bin_args.append(benchmark_info.ava6_binary)
...
def run_regression(
    ...
    """Determines the expected output for a benchmark, runs ava6 on it using
    ...
    if not os.access(ava6_binary, os.X_OK):
        sys.exit('"{}" does not exist or is not executable'.format(ava6_binary))
    if not os.path.isfile(benchmark_path):
        sys.exit('"{}" does not exist or is not a file'.format(benchmark_path))

    ava6_features, ava6_disabled_features = get_ava6_features(ava6_binary,
                                                              timeout)
    if ava6_features is None:
        return EXIT_TIMEOUT
    ...
    if benchmark_ext == ".smt2":
        ...
        sys.exit('"{}" must be *.smt2'.format(benchmark_basename))
    ...
    if "AVA6_REGRESSION_ARGS" in os.environ:
        basic_command_line_args += shlex.split(os.environ["AVA6_REGRESSION_ARGS"])
    ...
    # Run ava6 on the benchmark with the different testers and check whether
...
def main():
    ...
    parser.add_argument("ava6_binary")
    ...
    ava6_binary = os.path.abspath(g_args.ava6_binary)
