 ...
 * ****************************************************************************
 *
 * Black box testing of ava6::SymbolTable.
 */
...
{
  SymbolTable symtab;
  ava6::Sort booleanType = d_tm.getBooleanSort();
  ava6::Term x = d_tm.mkConst(booleanType);
  symtab.bind("x", x);
...
TEST_F(TestNodeBlackSymbolTable, bind2)
{
  ...
  ava6::Term y = d_tm.mkConst(booleanType, "y");
  symtab.bind("x", y);
  ASSERT_TRUE(symtab.isBound("x"));
...
TEST_F(TestNodeBlackSymbolTable, bind3)
{
  ...
  ava6::Term x = d_tm.mkConst(booleanType);
  symtab.bind("x", x);
  ava6::Term y = d_tm.mkConst(booleanType);
...
TEST_F(TestNodeBlackSymbolTable, bind4)
{
  ...
  ava6::Sort t = d_tm.mkUninterpretedSort("T");
...
{
  SymbolTable symtab;
  ava6::Sort s = d_tm.mkUninterpretedSort("S");
  symtab.bindType("S", s);
