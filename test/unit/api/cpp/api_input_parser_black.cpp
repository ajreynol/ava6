 ...
 * ****************************************************************************
 *
 * Black box testing of ava6::parser::InputParser SMT-LIbv2 inputs.
 */
...
{
  InputParser p(d_solver.get());
  ASSERT_THROW(p.nextCommand(), Ava6ApiException);
  std::stringstream ss;
  p.setStreamInput(modes::InputLanguage::SMT_LIB_2_6, ss, "input_parser_black");
...
{
  InputParser p(d_solver.get());
  ASSERT_THROW(p.nextTerm(), Ava6ApiException);
  std::stringstream ss;
  p.setStreamInput(modes::InputLanguage::SMT_LIB_2_6, ss, "input_parser_black");
...
TEST_F(TestApiBlackInputParser, multipleParsers)
{
  ...
  ASSERT_THROW(d_solver->setLogic("QF_LRA"), Ava6ApiException);
