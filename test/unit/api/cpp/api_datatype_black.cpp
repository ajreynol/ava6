...
TEST_F(TestApiBlackDatatype, mkDatatypeSort)
{
  ...
  ASSERT_THROW(d[2], Ava6ApiException);
...
TEST_F(TestApiBlackDatatype, mkDatatypeSorts)
{
  ...
  ASSERT_THROW(d_tm.mkDatatypeSorts(dtdeclsBad), Ava6ApiException);
}
...
TEST_F(TestApiBlackDatatype, datatypeStructs)
{
  ...
  Sort nullSort;
  ASSERT_THROW(cons.addSelector("null", nullSort), Ava6ApiException);
  dtypeSpec.addConstructor(cons);
  DatatypeConstructorDecl nil = d_tm.mkDatatypeConstructorDecl("nil");
  dtypeSpec.addConstructor(nil);
...
TEST_F(TestApiBlackDatatype, datatypeNames)
{
  ...
  ASSERT_THROW(dt.getSelector("cons"), Ava6ApiException);

  // possible to construct null datatype declarations if not using solver
  ASSERT_THROW(DatatypeDecl().getName(), Ava6ApiException);
}
...
TEST_F(TestApiBlackDatatype, isFinite)
{
  ...
  ASSERT_THROW(pdtype.getDatatype().isFinite(), Ava6ApiException);
}
...
TEST_F(TestApiBlackDatatype, datatypeSpecializedCons)
{
  ...
  ASSERT_THROW(nilc.getInstantiatedTerm(isort), Ava6ApiException);
}
}  // namespace test
}  // namespace ava6::internal
