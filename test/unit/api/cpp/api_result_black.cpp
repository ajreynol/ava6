...
TEST_F(TestApiBlackResult, equalHash)
{
  ...
  d_solver->assertFormula(x.eqTerm(x));
  ava6::Result res;
  ava6::Result res2 = d_solver->checkSat();
  ava6::Result res3 = d_solver->checkSat();
  ASSERT_NE(res, res2);
  res = res2;
  ASSERT_EQ(res, res2);
  ASSERT_EQ(res3, res2);
  ...
  ASSERT_EQ(std::hash<ava6::Result>{}(res), std::hash<ava6::Result>{}(res2));
  ASSERT_NE(std::hash<ava6::Result>{}(ava6::Result()),
            std::hash<ava6::Result>{}(res2));
...
TEST_F(TestApiBlackResult, isUnknown)
{
  ...
  ava6::UnknownExplanation ue = res.getUnknownExplanation();
  ASSERT_EQ(ue, ava6::UnknownExplanation::INCOMPLETE);
  {
    std::stringstream ss;
    ss << ue;
