...
def test_set_file_input(solver):
    ...
    with pytest.raises(RuntimeError):
        p.setFileInput(ava6.InputLanguage.SMT_LIB_2_6, "nonexistent.smt2")

def test_set_and_append_incremental_string_input(tm, solver):
    sm = SymbolManager(tm)
    p = InputParser(solver, sm)
    p.setIncrementalStringInput(ava6.InputLanguage.SMT_LIB_2_6, "test_input_parser")
...
def test_set_string_input(tm, solver):
    sm = SymbolManager(tm)
    p = InputParser(solver, sm)
    p.setStringInput(ava6.InputLanguage.SMT_LIB_2_6, "(set-logic ALL)", "test_input_parser")
    cmd = p.nextCommand()
...
def test_next_term2(tm, solver):
    ...
    with does_not_raise():
        t = p.nextTerm()
    assert t.isNull() is False
    assert t.getKind() == ava6.Kind.ADD
...
def test_multiple_parsers(tm, solver):
    ...
    p3.setIncrementalStringInput(ava6.InputLanguage.SMT_LIB_2_6, "test_input_parser")
    ...
    p4.setIncrementalStringInput(ava6.InputLanguage.SMT_LIB_2_6, "test_input_parser")
    ...
    with pytest.raises(RuntimeError):
        p5.setIncrementalStringInput(ava6.InputLanguage.SMT_LIB_2_6, "test_input_parser")
