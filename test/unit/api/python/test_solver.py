...
from ava6 import Kind, OptionCategory, SortKind, TermManager, Solver, Plugin
from ava6 import RoundingMode
from ava6 import BlockModelsMode, LearnedLitType, FindSynthTarget
from ava6 import ProofComponent, ProofFormat
...
def test_set_info(solver):
    with pytest.raises(RuntimeError):
        solver.setInfo("ava6-lagic", "QF_BV")
    with pytest.raises(RuntimeError):
        solver.setInfo("cvc2-logic", "QF_BV")
    with pytest.raises(RuntimeError):
        solver.setInfo("ava6-logic", "asdf")
...
def test_declare_sygus_var(tm, solver):
    ...
    with pytest.raises(RuntimeError):
        solver.declareSygusVar("", ava6.Sort())
    with pytest.raises(RuntimeError):
        solver.declareSygusVar("a", ava6.Sort())
    with pytest.raises(RuntimeError):
