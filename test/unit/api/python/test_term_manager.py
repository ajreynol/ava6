...
from ava6 import Kind, SortKind, RoundingMode, TermManager, Solver
...
def test_mk_skolem(tm):
    ...
    sk = tm.mkSkolem(ava6.SkolemId.ARRAY_DEQ_DIFF, a, b)
    sk2 = tm.mkSkolem(ava6.SkolemId.ARRAY_DEQ_DIFF, b, a)

    assert sk.isSkolem()
    assert sk2.isSkolem()
    assert sk.getSkolemId() == ava6.SkolemId.ARRAY_DEQ_DIFF
    assert sk2.getSkolemId() == ava6.SkolemId.ARRAY_DEQ_DIFF
...
def test_skolem_num_indices(tm):
    num = tm.getNumIndicesForSkolemId(ava6.SkolemId.ARRAY_DEQ_DIFF)
    assert num == 2
