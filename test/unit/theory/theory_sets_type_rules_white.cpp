...
TEST_F(TestTheoryWhiteSetsTypeRuleApi, singleton_term)
{
  ...
  Term singletonInt = d_tm.mkTerm(ava6::Kind::SET_SINGLETON, {integerOne});
  Term singletonReal = d_tm.mkTerm(ava6::Kind::SET_SINGLETON, {realOne});
  // (union
  //    (singleton (singleton_op Int) 1)
  //    (as emptyset (Set Real)))
  ASSERT_THROW(d_tm.mkTerm(ava6::Kind::SET_UNION, {singletonInt, emptyReal}),
               Ava6ApiException);
  // (union
  //    (singleton (singleton_op Real) 1)
  //    (as emptyset (Set Real)))
  ASSERT_NO_THROW(
      d_tm.mkTerm(ava6::Kind::SET_UNION, {singletonReal, emptyReal}));
}
